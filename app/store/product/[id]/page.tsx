                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {["64GB", "128GB", "256GB", "512GB"].map((storage) => (
                        <Button
                          key={storage}
                          variant={storage === "128GB" ? "default" : "outline"}
              ...
              <TabsContent value="description" className="mt-6">
                <div className="prose max-w-none">
                  ...
                  <ul>
                    ...
                    <li>Advanced camera system for capturing life's moments</li>
                  </ul>
                </div>
              </TabsContent>
...
function getProductById(id: string) {
  ...
  const brands = {
    ...
    accessories: ["Anker", "Belkin", "Logitech", "JBL", "Sony"],
    parts: ["Samsung", "Western Digital", "Corsair", "Kingston", "Seagate"],
  };

  const brand = brands[category]
    ? brands[category][Number.parseInt(index) % brands[category].length]
    : "Generic Brand";
  ...
  switch (category) {
    case "smartphones":
      name = `${brand} Phone ${index} Pro`;
      description =
        "A powerful smartphone with cutting-edge features and exceptional performance.";
      price = 500 + Number.parseInt(index) * 50;
      break;
    case "laptops":
      name = `${brand} Laptop ${index} Ultra`;
      description =
        "A high-performance laptop designed for productivity and entertainment.";
      price = 800 + Number.parseInt(index) * 100;
      break;
    case "tablets":
      name = `${brand} Tablet ${index}`;
      description = "A versatile tablet perfect for work and play.";
      price = 300 + Number.parseInt(index) * 40;
      break;
    case "accessories":
      ...
      const accessoryType =
        accessoryTypes[Number.parseInt(index) % accessoryTypes.length];
      name = `${brand} ${accessoryType}`;
      description = `A premium ${accessoryType.toLowerCase()} designed to enhance your tech experience.`;
      price = 50 + Number.parseInt(index) * 10;
      break;
    case "parts":
      const partTypes = ["SSD 1TB", "RAM 16GB", "Graphics Card", "Processor"];
      const partType = partTypes[Number.parseInt(index) % partTypes.length];
      name = `${brand} ${partType}`;
      description = `A high-quality ${partType.toLowerCase()} for upgrading your device.`;
      price = 70 + Number.parseInt(index) * 20;
      break;
    default:
      name = `Product ${index}`;
      description = "A quality product for your needs.";
      price = 100 + Number.parseInt(index) * 10;
  }

  const isNew = Number.parseInt(index) % 7 === 0;
  const discount =
    Number.parseInt(index) % 5 === 0
      ? 15
      : Number.parseInt(index) % 3 === 0
      ? 10
      : 0;
  const originalPrice = discount > 0 ? price * (100 / (100 - discount)) : price;
  const rating = 3 + (Number.parseInt(index) % 3);
  const reviews = 10 + Number.parseInt(index) * 5;
  const stock =
  ...
  return {
    ...
    stock,
    color:
      Number.parseInt(index) % 4 === 0
        ? "Black"
        : Number.parseInt(index) % 3 === 0
        ? "Silver"
        : Number.parseInt(index) % 2 === 0
        ? "Gold"
        : "Blue",
  };
}
